...
def generate_subtitles(
    ...
    if not subtitle_text or not timestamps:
        logger.warning("generate_subtitles: missing input — text=%r ts=%d", bool(subtitle_text), len(timestamps) if timestamps else 0)
        return None
    ...
    if not lines:
        logger.warning("generate_subtitles: no lines after split")
        return None

    # Debug: how many timestamp entries and chars
    ts_total_chars = sum(len(t.get("text", "")) for t in timestamps)
    logger.info(
        "generate_subtitles: %d lines, %d timestamps, %d ts_chars, total_text_chars=%d",
        len(lines), len(timestamps), ts_total_chars, len(subtitle_text.strip()),
    )
