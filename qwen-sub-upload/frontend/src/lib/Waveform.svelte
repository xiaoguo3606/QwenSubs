  ...
  $effect(() => {
    ...
    if (!wavesurfer) {
      wavesurfer = WaveSurfer.create({
        ...
        cursorColor: 'rgba(226, 232, 240, 0.3)',
        barWidth: 2,
        barRadius: 3,
        height: 100,
        normalize: true,
  ...
  <div class="controls">
    ...
    <div class="controls-right">
      <div class="time-info">
        ...
        <span class="time-value muted">{formatTime($duration)}</span>
      </div>
    </div>
  </div>
</div>
...
<style>
  ...
  .waveform { width: 100%; }
  .waveform :global(wavesurfer) {
    --ws-cursor-color: rgba(240, 236, 228, 0.3) !important;
  }
  .loading {
    position: absolute;
    top: 50%; left: 50%;
  ...
  .time-label { font-size: 0.7rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.04em; }
  .time-value {
    font-size: 0.85rem;
    font-weight: 600;
    color: var(--text-primary);
  ...
  .time-value.muted { color: var(--text-muted); }
</style>
