"""Alignment pipeline with progress callback support."""
...
def run_align_pipeline(
    ...
    if wav_file and wav_file.name != "converted.wav":
        try:
    ...
    def _format_line(line: str) -> str:
        ...
        if strip_mid_punct and mid_punct_choices:
            out = strip_mid_punctuation(out, mid_punct_choices, space_replacement)
        if strip_punct:
            out = strip_trailing_punctuation(out)
        if capitalize:
            out = capitalize_lines(out)
        return out.strip()

    processed_entries = [
        (_format_line(entry[0]), entry[1], entry[2])
        for entry in subtitle_entries
        if entry[0].strip()
    ]
    si_text = "\n".join(t for t, _, _ in processed_entries if t)

    result = {
        "text": si_text,
        "timestamps": timestamps,
        "subtitle_entries": [
            {"text": t, "start_time": start, "end_time": end}
            for t, start, end in processed_entries
            if t
        ],
    }
    if fail_count > 0:
