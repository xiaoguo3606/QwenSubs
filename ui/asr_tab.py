...
from core.audio_processor import is_audio_file, convert_to_16k_mono, get_audio_duration, split_audio, extract_audio_segment, MAX_AUDIO_DURATION
...
def create_asr_tab(registry_state: gr.State, timestamps_state: gr.State,
    ...
    with gr.Blocks() as tab:
        ...
        def _on_audio_upload(audio_path: str) -> tuple[str, gr.update]:
            status_msg = _validate_audio(audio_path)
            if audio_path:
                try:
    ...
    _i18n = [
        ...
        ("asr.split_point.label", split_point, "label"),
        ("asr.split_point.info", split_point, "info"),
    ]
...
def _run_asr(registry, audio_path, hint_text, split_point, language, strip_punct, capitalize,
    ...
    try:
        ...
        for seg_idx, (seg_start, seg_dur, seg_offset) in enumerate(segments):
            ...
            # Extract segment WAV (re-use wav_path for single segment)
            if len(segments) == 1:
                seg_path = wav_path
            else:
            ...
            chunks = split_audio(seg_path, max_duration=270.0)
            ...
            for idx, (chunk_path, chunk_start, _) in enumerate(chunks):
                ...
                # Progress: distribute across segments and chunks
                num_segs = len(segments)
                base_progress = seg_idx / num_segs
                rel_progress = (idx + 0.5) / len(chunks) / num_segs
                progress(base_progress + rel_progress, desc=f"识别第 {idx+1}/{len(chunks)} 段...")
                ...
                if results:
                    for r in results:
                        ...
                        if not ts_list:
                            ...
                            continue
                        r_text = r.get("text", "")
                        ts_text = "".join(ts["text"] for ts in ts_list)
                        _SPLIT_CHARS = set("，。！？、；：""「」『』【】（）()《》<>.!?,;:…—～\n")
                        parts = []
                        ts_idx = 0
                        for ch in r_text:
                            if ch in _SPLIT_CHARS:
                                ...
                                if ts_idx < len(ts_text):
                                    parts.append(ts_text[ts_idx])
                                    ts_idx += 1
                        if ts_idx < len(ts_text):
                            parts.append(ts_text[ts_idx:])
                ...
                progress(base_progress + (idx + 1) / len(chunks) / num_segs,
        ...
        logger.info(
            "_run_asr: %d chars text, %d timestamps, %d ts_chars",
            len(text), len(timestamps),
            sum(len(t.get("text", "")) for t in timestamps),
        )
    except Exception as e:
        return f"识别失败: {e}", timestamps_state, raw_text_state, subtitle_text_state
    finally:
        # Clean up temp files (chunks + converted wav)
